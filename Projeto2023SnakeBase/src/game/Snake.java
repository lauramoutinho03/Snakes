...
public abstract class Snake extends Thread implements Serializable{
	...
	protected void move(Cell cell) throws InterruptedException {
		// TODO
		if(getLength() < size) { // faz crescer a snake de acordo com o size
			BoardPosition newb = cell.getPosition();
			cells.addFirst(board.getCell(newb));
		} else { // mantem o size
			BoardPosition newb = cell.getPosition();
			cells.addFirst(board.getCell(newb));
	...
	public Boolean isOwnSnake(Cell c) {
		for(Cell cell: cells) {
			if(c.equals(cell)) {
				return true;
			} 
